...
def bfs_steps(grid, start=_UNSET, goal=_UNSET):
    ...
    reached  = set(sources)            # tập đã phát hiện (= visited trong spec)
    parent   = {}                      # truy vết đường đi

    # Kiểm tra goal ngay từ nguồn (khớp mã giả AIMA)
    if goal is not None:
        for s in sources:
            if s == goal:
    ...
    while frontier:
        current = frontier.popleft()   # POP

        # Yield state hiện tại TRƯỚC khi mở rộng (để scene hiển thị)
        # path chỉ set khi tìm thấy goal
        found_goal = False
        ...
        for cell in grid.neighbors(*current):
            ...
            # Kiểm tra goal khi phát hiện (early-exit – tối ưu cho BFS)
...
def dfs_steps(grid, start=_UNSET, goal=_UNSET):
    """DFS – dùng LIFO stack, đi sâu một hướng trước. Không đảm bảo
    tìm đường ngắn nhất nhưng tiết kiệm bộ nhớ frontier."""
    start = grid.start if start is _UNSET else start
    goal  = grid.goal  if goal  is _UNSET else goal

    sources = _resolve_sources(grid, start)
    stack   = list(sources)            # LIFO stack (append/pop)
    reached = set()                    # chỉ thêm khi thực sự pop (graph DFS)
    parent  = {}

    while stack:
        current = stack.pop()          # POP (LIFO)

        if current in reached:         # bỏ qua nếu đã xử lý
            continue
        reached.add(current)
...
def ids_steps(grid, start=_UNSET, goal=_UNSET, max_depth=40):
    ...
    overall_visited tích lũy qua các vòng lặp để visualize đúng."""
    start = grid.start if start is _UNSET else start
    goal  = grid.goal  if goal  is _UNSET else goal
    ...
    for depth_limit in range(max_depth + 1):
        # Mỗi vòng lặp là một DEPTH-LIMITED-SEARCH riêng biệt
